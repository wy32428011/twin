...
async function _exportProject(editor: Editor, options: IExportProjectOptions): Promise<void> {
	...
	if (!options.noDialog) {
		dialog = await showExportSceneProgressDialog(editor, "正在导出场景...");
	}
	...
	const checkIsForEditor = (obj: any): boolean =>
		!!(obj.metadata?.clonedFrom || (obj.parent && checkIsForEditor(obj.parent)));

	// 记录被 clone 的原始模型的 enabled 状态，导出前强制启用
	const originalEnabledStates = new Map<string, boolean>();
	scene.meshes.forEach((mesh) => {
		if (mesh.metadata?.cloneForEditor) {
			originalEnabledStates.set(mesh.id, mesh.isEnabled());
			mesh.setEnabled(true);
		}
	});

	scene.meshes.forEach((mesh) => (mesh.doNotSerialize = mesh.metadata?.doNotSerialize || checkIsForEditor(mesh) || false));
	scene.lights.forEach((light) => (light.doNotSerialize = light.metadata?.doNotSerialize || checkIsForEditor(light) || false));
	scene.cameras.forEach((camera) => (camera.doNotSerialize = camera.metadata?.doNotSerialize || checkIsForEditor(camera) || false));
	scene.transformNodes.forEach((transformNode) => (transformNode.doNotSerialize = transformNode.metadata?.doNotSerialize || checkIsForEditor(transformNode) || false));
	...
	// 恢复原始模型的 enabled 状态
	originalEnabledStates.forEach((wasEnabled, id) => {
