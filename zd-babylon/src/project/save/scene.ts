...
export async function saveScene(editor: Editor, projectPath: string, scenePath: string): Promise<void> {
  ...
  function checkIsForEditor(obj: any): boolean {
    return !!(obj.metadata?.clonedFrom || (obj.parent && checkIsForEditor(obj.parent)));
  }
    ...
    meshesToSave.map(async (mesh) => {
      ...
      const results = await Promise.all(
        meshes.map(async (meshToSerialize) => {
          ...
          // --- 新增逻辑开始 ---
          // 1. 记录原始的启用状态
          const originalIsEnabled = meshToSerialize.isEnabled();

          // 2. 在序列化前强制启用，被clone的原生Mesh需要强制启用
          if (meshToSerialize.metadata?.cloneForEditor)
            meshToSerialize.setEnabled(true);

          // 3. 执行序列化
          const data = await SceneSerializer.SerializeMesh(meshToSerialize, false, false);

          // 4. (可选) 序列化完成后，恢复原始状态
          // 如果你希望保存操作完全不影响当前运行时的视图，建议加上这一行
          meshToSerialize.setEnabled(originalIsEnabled);
          // --- 新增逻辑结束 ---
  ...
  await Promise.all(
    scene.transformNodes.map(async (transformNode) => {
      if (!isTransformNode(transformNode) || isFromSceneLink(transformNode) || checkIsForEditor(transformNode)) {
        return;
      }
  ...
  getBufferSceneScreenshot(scene).then((screenshotBuffer) => {
    if (screenshotBuffer) { writeFile(join(scenePath, "preview.png"), screenshotBuffer); }
  });
