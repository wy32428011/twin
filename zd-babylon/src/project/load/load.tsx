...
import { normalizeMqttConfiguration } from "../../tools/mqtt/configuration";
...
export async function loadProject(editor: Editor, path: string) {
  ...
  editor.setState({
    ...
    mqtt: normalizeMqttConfiguration(project.mqtt),
  });
  ...
  if (project.lastOpenedScene) {
    ...
    if (!(await pathExists(absolutePath))) {
      toast(`Scene "${project.lastOpenedScene}" 不存在。`);

      return editor.layout.console.error(`Scene "${project.lastOpenedScene}" 不存在。`);
    }
    ...
    await editor.layout.preview.editorPlay.init();
  }
}
...
export async function checkDependencies(
  ...
  if (installCode !== 0) {
    toast.warning(`包管理器 "${packageManager}" 在您的系统上不可用，依赖项将不会更新。`);
  }
...
export async function loadProjectPlugins(editor: Editor, path: string, project: IEditorProject) {
  for (const plugin of project.plugins) {
    try {
      ...
      if (isLocalPlugin) {
        editor.layout.console.log(`已从本地加载插件 "${result.title ?? plugin.nameOrPath}"`);
      } else {
        editor.layout.console.log(`已加载插件 "${result.title ?? plugin.nameOrPath}"`);
      }
    } catch (e) {
      console.error(e);
      editor.layout.console.error(`加载插件失败 "${plugin.nameOrPath}"`);
    }
  }
}
