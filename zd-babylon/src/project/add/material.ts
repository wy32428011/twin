...
import { GridMaterial, NormalMaterial, WaterMaterial, LavaMaterial, TriPlanarMaterial, CellMaterial, FireMaterial, GradientMaterial } from "babylonjs-materials";
...
export function addSkyMaterial(scene: Scene) {
	const material = new NodeMaterial("New Sky Material", scene);
	material.id = Tools.RandomId();
	material.uniqueId = UniqueNumber.Get();
