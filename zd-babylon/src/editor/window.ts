...
export async function createEditorWindow(): Promise<BrowserWindow> {
  ...
  const window = new BrowserWindow({
    ...
    webPreferences: {
      ...
      contextIsolation: process.env.DEBUG !== "true" && process.env.DEBUG_BUILD !== "true",
  ...
  if (process.env.DEBUG !== "true" && process.env.DEBUG_BUILD !== "true") {
  ...
  if (process.env.DEBUG || process.env.DEBUG_BUILD) {
    setTimeout(() => {
