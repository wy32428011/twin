import { Component, DragEvent, JSX, ReactNode } from "react";
...
import { BaseTexture, Node, Sound, Tools, IParticleSystem, Sprite } from "babylonjs";
...
export class EditorGraph extends Component<IEditorGraphProps, IEditorGraphState> {
	...
	public setSelectedNode(node: Node | Sound | IParticleSystem | Sprite): void {
		let targetNode: Node | Sound | IParticleSystem | Sprite = node;

		if (isNode(node) && (node as any).metadata?.clonedFrom) {
			const original = this.props.editor.layout.preview.scene.getMeshById((node as any).metadata.clonedFrom);
			if (original) {
				targetNode = original;
			}
		}

		let source = isSound(targetNode) ? targetNode["_connectedTransformNode"] : isAnyParticleSystem(targetNode) ? targetNode.emitter : targetNode;
		...
		this._forEachNode(this.state.nodes, (n) => {
			...
			n.isSelected = n.nodeData === targetNode;
		});
	...
	private _handleNodeClicked(node: TreeNodeInfo, ev: React.MouseEvent<HTMLElement>): void {
		if (!this._isNodeInScene(node.nodeData)) {
			return;
		}
	 ...
	 * 检查节点是否仍在场景容器内（未被删除）。
	 */
	private _isNodeInScene(nd: any): boolean {
		const scene = this.props.editor.layout.preview.scene;

		if (isAbstractMesh(nd)) return scene.meshes.indexOf(nd) !== -1;
		if (isLight(nd)) return scene.lights.indexOf(nd) !== -1;
		if (isCamera(nd)) return scene.cameras.indexOf(nd) !== -1;
		if (isAnyTransformNode(nd)) return scene.transformNodes.indexOf(nd) !== -1;
	...
	private _parseSceneNode(node: Node, noChildren?: boolean): TreeNodeInfo | null {
		if ((isMesh(node) && (node._masterMesh || !isNodeVisibleInGraph(node))) || isCollisionMesh(node) || isCollisionInstancedMesh(node) || (node as any).metadata?.clonedFrom) {
			return null;
		}
		...
		if(node.name === 'sky' || node.name === 'ground') {
			return null;
		}
	...
	private _getNodeIconComponent(node: Node): ReactNode {
		return (
			<div
				...
				className={`cursor-pointer ${node.isEnabled() || (node as any).metadata?.cloneForEditor ? "opacity-100" : "opacity-20"} transition-all duration-100 ease-in-out`}
			>
				{this._getIcon(node)}
			</div>
		);
	...
	private _getNodeLabelComponent(object: any, name?: string | null, noContextMenu?: boolean): JSX.Element {
		...
		return (
			<EditorGraphContextMenu editor={this.props.editor} object={object} from="graph">
				{label}
