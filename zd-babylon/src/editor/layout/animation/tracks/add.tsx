...
export function EditorAnimationAddTrackPrompt(props: IEditorAnimationAddTrackPromptProps) {
	...
	return (
		<AlertDialog open>
			<AlertDialogContent>
				<AlertDialogHeader>
					<AlertDialogTitle>添加新轨道</AlertDialogTitle>
					<AlertDialogDescription></AlertDialogDescription>
				</AlertDialogHeader>

				<Command className="border-muted border-[1px]">
					<CommandInput ref={(r) => r?.focus()} onValueChange={(v) => setValue(v)} placeholder="搜索要动画化的属性..." />
					<CommandList>
						<CommandEmpty>动画化 "{value}"</CommandEmpty>
				...
				<AlertDialogFooter>
					<AlertDialogCancel onClick={() => props.onSelectProperty(null)}>取消</AlertDialogCancel>
					<AlertDialogAction onClick={() => props.onSelectProperty(value)}>确认</AlertDialogAction>
				</AlertDialogFooter>
			</AlertDialogContent>
		</AlertDialog>
	);
