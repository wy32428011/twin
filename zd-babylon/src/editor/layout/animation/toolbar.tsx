		...
		return (
			<div className="flex justify-between items-center w-full h-10 bg-primary-foreground">
				<Menubar className="border-none rounded-none pl-3 my-auto bg-primary-foreground h-10">
					{/* File */}
					<MenubarMenu>
						<MenubarTrigger disabled={this.props.animatable === null}>文件</MenubarTrigger>

						<MenubarContent className="border-black/50">
							<MenubarItem onClick={() => importAnimationsFrom(this.props.animationEditor, this.props.animatable)}>加载动画自...</MenubarItem>
							<MenubarSeparator />
							<MenubarItem onClick={() => exportAnimationsAs(this.props.animatable)}>另存动画为...</MenubarItem>
						</MenubarContent>
					</MenubarMenu>

					{/* Edit */}
					<MenubarMenu>
						<MenubarTrigger disabled={this.props.animatable === null}>编辑</MenubarTrigger>

						<MenubarContent className="border-black/50">
							<MenubarItem onClick={() => this.props.animationEditor.timelines.addKeysAtCurrentTime()}>在当前时间添加关键帧</MenubarItem>
						</MenubarContent>
					</MenubarMenu>
				</Menubar>
