...
export function removeNodes(editor: Editor) {
	...
	for (const d of nodes) {
		if (d.node.metadata?.cloneForEditor) {
			cloneMap.set(d.node.uniqueId, d.node.metadata.cloneForEditor);
		}
	}
	...
	registerUndoRedo({
		...
		undo: () => {
			...
			// 重建克隆体
			for (const d of nodes) {
				if (cloneMap.has(d.node.uniqueId) && isMesh(d.node) && !d.node.parent) {
					editor.layout.preview.editorPlay.hotReloadScriptForNotPlaying("", false, d.node);
				}
			}
		},
		redo: () => {
			// 销毁克隆体
			for (const d of nodes) {
				if (cloneMap.has(d.node.uniqueId)) {
					editor.layout.preview.editorPlay.disposeCloneMesh(d.node);
				}
			}
