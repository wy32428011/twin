...
export async function exportScene(editor: Editor): Promise<void> {
	const filePath = saveSingleFileDialog({
		title: "导出场景",
		filters: [{ name: "Babylon 场景文件", extensions: ["babylon"] }],
	...
	try {
		...
		editor.layout.console.log(`场景已成功导出至 ${filePath}`);
		toast.success(`场景已成功导出至 ${filePath}`);
	} catch (e) {
		if (e instanceof Error) {
			editor.layout.console.error(`导出场景失败: ${e.message}`);
			toast.error("导出场景失败");
		}
	}
}
...
export async function exportNodeToPath(editor: Editor, node: Node, filePath: string): Promise<void> {
	try {
		...
		editor.layout.console.log(`节点已成功导出至 ${filePath}`);
		toast.success(`节点已成功导出至 ${filePath}`);
	} catch (e) {
		if (e instanceof Error) {
			editor.layout.console.error(`导出节点失败: ${e.message}`);
			toast.error("导出节点失败");
		}
	}
}
