...
import { ICinematic } from "../../editor-tools";
...
import { Button, TreeNodeInfo } from "@blueprintjs/core";
...
export class EditorAssetsBrowser extends Component<IEditorAssetsBrowserProps, IEditorAssetsBrowserState> {
	...
	public componentDidMount(): void {
		onProjectConfigurationChangedObservable.add((c) => {
			if (c.path) {
				const rootUrl = join(dirname(c.path), 'assets');
			...
			<div className="flex flex-col w-full h-full">
				<div className="flex gap-2 justify-between w-full h-10 min-h-10 bg-primary-foreground">
					<div className="flex gap-2 h-full">
						...
						<Button minimal icon="import" text="导入" onClick={() => this._handleImportFiles()} />
					</div>
	...
	private _getBreadcrumbComponent(): ReactNode {
		...
		const rootPath = join(dirname(projectConfiguration.path!), "/assets");
				...
				<ContextMenu>
					...
					<ContextMenuContent>
						<ContextMenuItem className="flex items-center gap-2" onClick={() => this._refreshItems(this.state.browsedPath!)}>
							<IoRefresh className="w-5 h-5" /> 刷新
						</ContextMenuItem>
						...
						<ContextMenuItem className="flex items-center gap-2" onClick={() => this._handleCreateDirectory()}>
							<AiOutlinePlus className="w-5 h-5" /> 新建目录
						</ContextMenuItem>
					</ContextMenuContent>
				</ContextMenu>
			</SelectableGroup>
	...
	private _getAssetsContextMenuItems(): ReactNode {
		return (
			<>
				<ContextMenuItem onClick={() => this._handleAddScene()}>场景</ContextMenuItem>
				<ContextMenuSeparator />
				...
				<ContextMenuSeparator />
				<ContextMenuItem onClick={() => this._handleAddNodeMaterialFromSnippet()}>从代码片段创建节点材质...</ContextMenuItem>
				<ContextMenuSeparator />

				<ContextMenuSub>
					<ContextMenuSubTrigger className="flex items-center gap-2">材质库</ContextMenuSubTrigger>
					<ContextMenuSubContent>
				...
				{this.props.editor.state.enableExperimentalFeatures && (
					<>
						<ContextMenuSeparator />
						<ContextMenuItem onClick={() => this._handleAddNodeParticleSystem()}>节点粒子系统</ContextMenuItem>
						<ContextMenuSeparator />
						<ContextMenuItem onClick={() => this._handleAddCinematic()}>电影级</ContextMenuItem>
						{/* <ContextMenuSeparator /> */}
				...
				{this.props.editor.state.enableExperimentalFeatures && (
					<>
						<ContextMenuSeparator />
						<ContextMenuItem onClick={() => this._handleAddFullScreenGUI()}>全屏 GUI</ContextMenuItem>
					</>
				)}
			</>
		);
	...
	private _getSrcContextMenuItems(): ReactNode {
		return (
			<>
				<ContextMenuItem onClick={() => this._handleAddScript("class")}>类脚本</ContextMenuItem>
				<ContextMenuItem onClick={() => this._handleAddScript("function")}>函数脚本</ContextMenuItem>
			</>
		);
	}
	...
	private async _handleDrop(event: DragEvent<HTMLDivElement>): Promise<void> {
		...
		if (assetFileNotInAssetsFolder) {
			showAlert(
				"警告",
				<div>
					您尝试导入的资源不在根目录 "assets" 文件夹中。
					<br />
					为防止引用断裂，这些文件未被复制。请将它们拖放到 /assets 文件夹中。
				</div>,
				true
			);
		}
	...
	private async _handleDroppedNodesFromGraph(nodeIds: string[]): Promise<void> {
		...
		if (!this.isAssetsFolder()) {
			showAlert("警告", <div>您只能在根目录 "assets" 文件夹中导出节点。</div>, true);
			return;
		}
		...
		for (const nodeId of nodeIds) {
			...
			if (node) {
				...
				if (await pathExists(scenePath)) {
					const overwrite = await showConfirm("覆盖场景？", `名为 "${node.name}" 的场景已存在于此位置。是否要覆盖它？`);
					if (!overwrite) {
						continue;
					}
				}
	...
	private async _handleImportFiles(): Promise<void> {
		...
		const folder = openSingleFolderDialog('选择文件夹');
		const fStat = await stat(folder);

		const destination = join(this.state.browsedPath!, basename(folder));

		if (fStat.isDirectory()) {
			await copy(folder, destination, {
				recursive: true,
			});
		} else {
			showAlert("警告", (<div>请选择文件夹</div>), true);
		}
	...
	private async _handleAddScene(): Promise<void> {
		...
		let name = await showPrompt("场景名称", "输入新场景的名称", "新场景");
		if (!name) {
			return;
		}
		...
		const openResult = await showConfirm("打开新场景", "要立即打开新场景吗？");
		if (openResult) {
	...
	private async _handleAddNodeMaterialFromSnippet(): Promise<void> {
		...
		const id = await showPrompt("代码片段 ID", "输入要导入的节点材质代码片段 ID");
		if (!id) {
			return;
		}
	...
	// 	return join(this.state.browsedPath!) === join(dirname(projectConfiguration.path!), 'assets');
	// }
	...
	private async _handleLoadScene(absolutePath: string, newlyCreated?: boolean): Promise<void> {
		...
		if (!newlyCreated) {
			const accept = await showConfirm("确定要执行此操作吗？", "这将关闭当前场景并打开所选场景。");
			if (!accept) {
				return;
			}
		}

		const acceptSave = await showConfirm("保存当前场景？", "在打开新场景之前要保存当前场景吗？", {
			confirmText: "保存",
			cancelText: "不保存",
		});
