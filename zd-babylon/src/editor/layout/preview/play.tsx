...
// import { IRegisteredScript } from "editor-tools";
import { Node, Scene, Sprite } from "babylonjs";
...
export class EditorPreviewPlayComponent extends Component<IEditorPreviewPlayComponentProps, IEditorPreviewPlayComponentState> {
			...
			<TooltipProvider>
				{this.state.playing && !this.state.preparingPlay && (
					<Tooltip>
						...
						<TooltipContent>重启游戏/应用</TooltipContent>
					</Tooltip>
				)}
				...
				<Tooltip>
					...
					<TooltipContent className="flex gap-2 items-center">
						{this.props.enabled ? "运行游戏/应用" : "依赖仍在安装中，暂时无法运行..."}
					</TooltipContent>
				</Tooltip>
			</TooltipProvider>
		);
	...
	private _startMqttTwinRuntime(): void {
		this._stopMqttTwinRuntime();

		if (!this.props.editor.state.mqtt.enabled || !this.scene) {
			return;
		}

		this._mqttTwinRuntime = new EditorMqttTwinRuntime({ scene: this.scene, mode: "play" });
		this._mqttTwinRuntime.start();
	}

	private _stopMqttTwinRuntime(): void {
	...
	public stop(): void {
		...
		this.props.editor.layout.preview.editorPlay.afterPlaying();
	...
	public async play(noExportScene?: boolean, noCompile?: boolean): Promise<void> {
		...
		this.props.editor.layout.preview.editorPlay.beforePlaying();
		...
		this._srcWatcher.on("change", async (path) => {
			if (this.canPlayScene) {
				...
				if (this.props.editor.state.enableExperimentalFeatures) {
					...
					await this.hotReloadScript(scriptKey, true);
				} else {
					await this.restart();
				}
			}
	...
	public async hotReloadScript(scriptKey: string, compile: boolean): Promise<void> {
		if (!this.scene || !this.props.editor.state.enableExperimentalFeatures) {
			return;
		}
		...
		allNodes.forEach((n) => {
			const runningScripts = oldScriptExports.scriptsDictionary.get(n) as any[] | undefined;
			if (!runningScripts) {
				return;
			}
			...
			runningScriptsCopy.forEach(async (script) => {
				...
				const sourceMetadata = sourceObject?.metadata?.scripts?.find((sourceScript: any) => sourceScript.key === script.key);
