...
export class EditorPreviewAxisHelper extends Component<IEditorPreviewAxisHelperProps, IEditorPreviewAxisHelperState> {
	...
	public render(): ReactNode {
		...
		// 此处top的变化值(90%)，需要是viewport的变化(0.95)的两倍
		// 因为viewport的量程是[-1 ~ 1]，而top的量程是[0 ~ 100%]，所以需要乘以2
		return (
			<div hidden={preview?.state.fixedDimensions !== "fit" || preview?.play?.state.playing}>
				<div
					...
					style={{
						top: `calc(calc(${this.state.xLabelPosition.y}px + 164px) - 90%)`,
				...
				<div
					className="absolute text-black/50 text-xs font-semibold -translate-x-1/2 -translate-y-1/2 pointer-events-none"
					style={{
						top: `calc(calc(${this.state.yLabelPosition.y}px + 164px) - 90%)`,
				...
				<div
					className="absolute text-black/50 text-xs font-semibold -translate-x-1/2 -translate-y-1/2 pointer-events-none"
					style={{
						top: `calc(calc(${this.state.zLabelPosition.y}px + 164px) - 90%)`,
	...
	public async start(): Promise<void> {
		...
		this.scene.onBeforeRenderObservable.add(() => {
			...
			camera.viewport = new Viewport(1 - width, 0.95 - height, width, height);
