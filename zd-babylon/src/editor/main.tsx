...
import "../tools/scene/printNodeTree";
...
import { createDefaultMqttConfiguration } from "../tools/mqtt/configuration";
...
import { EditorMqttSettingsDialog } from "./dialogs/mqtt/mqtt";
...
export class Editor extends Component<IEditorProps, IEditorState> {
  ...
  public render(): ReactNode {
    return (
      <>
        ...
        <Toaster /> 
        */}
        <EditorMqttSettingsDialog editor={this} open={this.state.editMqtt} onClose={() => this.setState({ editMqtt: false })} />
  ...
  public syncMqttConnection(): void {
    if (this.state.mqtt.enabled) {
      ipcRenderer.send("mqtt:connect", this.state.mqtt);
    } else {
      ipcRenderer.send("mqtt:disconnect");
    }

    this.layout?.preview?.startMqttTwinPreview();
  }
