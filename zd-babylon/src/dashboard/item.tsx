...
export function DashboardProjectItem(props: IDashboardProjectItemProps) {
  ...
  async function handleLaunchProject() {
    ...
    progressRef?.setState({ message: `正在安装依赖...` });
    ...
    progressRef?.setState({ message: `正在运行项目...` });
        ...
        >
          ...
          <div className="flex flex-col gap-1 p-2 bg-secondary rounded-b-lg select-none">
            ...
            <div className="text-muted-foreground text-xs">
              创建于 {new Date(props.project.createdAt).toLocaleString("zh-CN", { day: "2-digit", month: "long", year: "numeric" })}
            </div>
            <div className="text-muted-foreground text-xs">
              更新于 {new Date(props.project.updatedAt).toLocaleString("zh-CN", { day: "2-digit", month: "long", year: "numeric" })}
            </div>
          </div>
        </div>
      </ContextMenuTrigger>
      <ContextMenuContent>
        <ContextMenuItem onClick={handleLoadProject}>打开</ContextMenuItem>
